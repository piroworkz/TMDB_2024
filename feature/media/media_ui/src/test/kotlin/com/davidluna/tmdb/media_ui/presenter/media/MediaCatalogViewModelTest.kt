...
class MediaCatalogViewModelTest {
    ...
    fun `GIVEN (viewmodel initialization) WHEN (getContentKindUseCase collection succeeds) THEN (should update contentKind state = ContentKind)`() =
        runTest {
            ...
            whenever(getContentKind()).thenReturn(flowOf(com.davidluna.tmdb.core_domain.entities.ContentKind.MOVIE))

            val viewModel = buildViewModel()

            viewModel.state.test {
                assertThat(awaitItem()).isEqualTo(initialState)
                assertThat(awaitItem().contentKind).isEqualTo(com.davidluna.tmdb.core_domain.entities.ContentKind.MOVIE)
